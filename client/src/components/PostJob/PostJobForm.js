      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            <form onSubmit={handleSubmit(onSubmit)} className="p-4 sm:p-6 md:p-8 space-y-8">
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  <FormInput label="Title" register={register} name="title" error={errors?.title} placeholder="Ex: Software Engineer" />

                  <FormInput label="Location Type" register={register} name="locationType" type="select" options={FORM_OPTIONS.location} error={errors?.locationType} />

                  <FormInput label="Employment Type" register={register} name="type" type="select" options={FORM_OPTIONS.employment} error={errors?.type} />

                  <FormInput label="Schedule Type" register={register} name="scheduleType" type="select" options={FORM_OPTIONS.schedule} error={errors?.scheduleType} />
                  ...
                  <FormInput label="Hire Type" register={register} name="hireType" type="select" options={FORM_OPTIONS.hire} error={errors?.hireType} />

                  <FormInput label="Compensation" register={register} name="compensation" type="number" error={errors?.compensation} placeholder="Ex: 50000" />

                  <FormInput label="Experience Required (years)" register={register} name="experienceRequired" type="number" error={errors?.experienceRequired} placeholder="Ex: 3" />

                  <FormInput label="Required Number of Resources" register={register} name="requiredResources" type="number" error={errors?.requiredResources} placeholder="Ex: 5" />

                  <FormInput label="Status" register={register} name="status" type="select" options={FORM_OPTIONS.status} error={errors?.status} />

                  {/* Conditional Recruiter Field */}
                  {isHead && (
                    <FormField label="Recruiter Name" error={errors?.recruiterName}>
                      ...
                      >
                        ...
                        {recruitersList?.length > 0 ? recruitersList.map(recruiter => (
                          <option key={recruiter._id} value={recruiter._id}>{recruiter.userName}</option>
                        )) : (
                  ...
                  <FormField label="Hiring Manager Email" error={errors?.hiringManagerEmail}>
                    <select
                      {...register("hiringManagerEmail", { required: "Hiring Manager is required" })}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                    >
                      <option value="">Select Hiring Manager</option>
                      {hiringManagersList?.length > 0 ? hiringManagersList.map(manager => (
                        <option key={manager._id} value={manager.email}>{manager.email}</option>
                      )) : (
              ...
              <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
                <button
                  type="submit"
                  className="w-full sm:w-auto px-8 py-3 bg-gray-700 text-white font-medium hover:bg-gray-500 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors"
                >
